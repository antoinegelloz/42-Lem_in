...
/*   Updated: 2019/10/10 19:17:08 by agelloz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int8_t	all_ants_moved(t_list **ants_pos, ssize_t *tmp_pos, ssize_t ants, ssize_t actual_ant, t_paths *paths)
{
	ssize_t i;
	...
	while (i < ants)
	{
		if (ants_pos[i] != NULL && ants_pos[i] != paths->array[paths->ants_to_paths[i]])
		{
			//ft_printf("tmp_pos[%d] = %d\n", i, tmp_pos[i]);
			//ft_printf("ants_pos[%d] = %d\n", i, *(ssize_t *)ants_pos[i]->content);
			if (*(ssize_t *)ants_pos[i]->content == tmp_pos[i])
				return (FALSE);
		}
		i++;
	}
...
int8_t	all_possible_movements_finished(t_list **ants_pos, t_paths *paths, t_graph *graph, size_t ant)
{
	while (ant < graph->ants)
	{
		if (ants_pos[ant] == paths->array[paths->ants_to_paths[ant]] && paths->available[paths->ants_to_paths[ant]] == TRUE)
			return (FALSE);
		if (ants_pos[ant] != NULL && ants_pos[ant]->next != NULL)
			return (FALSE);
		ant++;
	}
	return (TRUE);
		...
		while (ant < graph->ants)
		{
			...
			if (ants_pos[ant] == paths->array[paths->ants_to_paths[ant]])
			{
				if (paths->available[paths->ants_to_paths[ant]] == TRUE)
				{
					...
					if (all_paths_used(paths, graph) == FALSE && !(ant == graph->ants - 1 && paths->output_lines == 1) && ant != graph->ants - 1)
			...
			else if (ants_pos[ant] != NULL)
			{
				if (ants_pos[ant]->next != NULL)
				{
					...
					if (ants_pos[ant] != NULL)
					{
						if (all_ants_moved(ants_pos, tmp_pos, graph->ants, ant, paths) == FALSE)
							ft_printf("L%d-%s ", ant + 1, graph->nodes[*(size_t *)ants_pos[ant]->content].name);
						else 
							ft_printf("L%d-%s", ant + 1, graph->nodes[*(size_t *)ants_pos[ant]->content].name);
					}
			...
			//if (ants_pos[ant] != NULL && ants_pos[ant]->next == NULL)
			//	ft_printf("ant:%d\n", ant);
			if (ants_pos[ant] != NULL && ants_pos[ant]->next == NULL && ant == graph->ants - 1)
			{
				//ft_putendl("end to null");
				ants_pos[ant] = ants_pos[ant]->next;
			}
			ant++;
		}
...
int8_t	flow_ants(t_graph *graph, t_paths *paths) 
{
	...
	while (i < graph->ants)
	{
		...
		if (j == graph->paths_count - 1 && j > 0 && tmp[j] == 0)
		{
			//ft_putendl("reset");
			reset_availability(paths, graph);
		}
